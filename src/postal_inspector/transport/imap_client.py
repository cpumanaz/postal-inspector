...
class IMAPFetcher:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.error("imap_connection_failed", error=error_msg, error_type=type(e).__name__)
            raise DeliveryError(f"IMAP connection failed: {error_msg}")
    ...
    async def fetch_new_messages(self) -> AsyncGenerator[tuple[str, bytes], None]:
        ...
        to remove from the upstream server. This ensures no email loss.
        ...
        try:
            ...
            self._last_error = error_msg
            self._consecutive_failures += 1
            logger.error("imap_connection_lost", error=error_msg, error_type=type(e).__name__)
            raise DeliveryError(f"IMAP connection lost: {error_msg}")

    async def delete_message(self, msg_id: str) -> None:
        """Delete a message from INBOX after saving locally.

        This removes the email from the upstream server - we rely on local storage.
        """
        if not self._client:
            raise DeliveryError("IMAP not connected")
        ...
        try:
            ...
            logger.info("email_deleted_from_upstream", msg_id=msg_id)
        except Exception as e:
